 ...
 *  непосредственно в сервис почты, последовательно передает этот объект набору третьих лиц,
...
public class UntrustworthyMailWorker implements MailService {
    ...
    MailService [] fakeMailServices;
    public UntrustworthyMailWorker (MailService [] fakeMailServices) {
        this.fakeMailServices = fakeMailServices.clone();
    ...
    public Sendable processMail (Sendable mail) {
        Sendable m = mail;
        for (MailService fs : fakeMailServices) {
            m = fs.processMail(m);
        }
