...
import static delk.baseJave.edu.mail.MainMail.AUSTIN_POWERS;
 ...
 * 2.2) Иначе, необходимо написать в лог сообщение с уровнем
 * INFO: Usual correspondence: from {from} to {to}
 */
public class Spy implements MailService{
    ...
    public Sendable processMail (Sendable mail) {
        ...
        MailMessage theMail = (MailMessage) mail;
        if (mail.getFrom().equals(AUSTIN_POWERS) || mail.getTo().equals(AUSTIN_POWERS)) {
            logger.log(Level.WARNING, "Detected target mail correspondence: from {0} to {1} \"{2}\"", new Object [] {mail.getFrom(), mail.getTo(), theMail.getMessage()});
        }
        else {
